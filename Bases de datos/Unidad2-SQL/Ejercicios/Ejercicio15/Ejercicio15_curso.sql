...
CREATE VIEW horario_lunes(numhora,nombreModulo,nombreProfesor) AS SELECT horario.numhora,modulos.nombre,profesores.nombre 
    FROM horario INNER JOIN modulos ON horario.cmodulo=modulos.cmodulo INNER JOIN impartir ON modulos.cmodulo=impartir.cmodulo INNER JOIN profesores ON impartir.cprofesor=profesores.cprofesor 
    WHERE horario.diasemana="Lunes" AND impartir.titular=1;

-- 7. Obtén un horario semanal que muestre día, hora, módulo, profesor ordenado por día y hora.
CREATE VIEW horario_semanal(diasemana,numhora,nombreModulo,nombreProferos) AS SELECT diasemana,numhora,modulos.nombre,profesores.nombre 
    FROM horario INNER JOIN modulos ON horario.cmodulo=modulos.cmodulo INNER JOIN impartir ON modulos.cmodulo=impartir.cmodulo INNER JOIN profesores ON impartir.cprofesor=profesores.cprofesor 
    GROUP BY diasemana,numhora;
...
-- 9. Realiza un listado con las calificaciones del alumno número 4 en la primera evaluación.
CREATE VIEW clasificaciones_alumno4_eva1(Modulo,Nombre,Apellido,Nota) AS SELECT cursadas.cmodulo,alumnos.nombre,alumnos.apellidos,cursadas.nota1 
    FROM alumnos INNER JOIN cursadas ON alumnos.numalumno=cursadas.numalumno WHERE alumnos.numalumno=4;

-- 10. Realiza un listado de los alumnos que han aprobado el módulo de desarrollo en la primera evaluación.
CREATE VIEW aprobados_des_eva1(Nombre,Apellidos,Nota) AS SELECT alumnos.nombre,alumnos.apellidos,cursadas.nota1 
    FROM alumnos INNER JOIN cursadas ON alumnos.numalumno=cursadas.numalumno 
    WHERE cursadas.nota1>=5;

-- 11. Realiza un listado que muestre cuantos alumnos han aprobado cada módulo en la primera evaluación.
CREATE VIEW modulos_aprobados_eva1(Nombre,Apellidos,Modulo) AS SELECT alumnos.nombre,alumnos.apellidos,modulos.nombre 
    FROM alumnos INNER JOIN cursadas ON alumnos.numalumno=cursadas.numalumno INNER JOIN modulos ON cursadas.cmodulo=modulos.cmodulo 
    WHERE nota1>=5
